...
import { Edit, Trash2, Calendar as CalendarIcon, Bell, Loader2 } from "lucide-react"; // Added Loader2
...
export function TaskList() {
  ...
  React.useEffect(() => {
    ...
    const q = query(tasksColRef, where("userId", "==", user.uid), orderBy("createdAt", "desc")); // Filter by userId and order

    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetchedTasks = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      } as Task)); // Map Firestore data to Task interface
 ...
 // --- Utility Functions ---
  ...
  const formatTimestamp = (timestamp: Timestamp | undefined, formatString: string): string => {
                  ...
                  tasks.map((task) => (
                    <TableRow key={task.id} data-state={task.completed ? "completed" : ""}>
                        ...
                        <TableCell className={task.completed ? 'line-through text-muted-foreground' : ''}>
                         ...
                         {task.deadline ? formatTimestamp(task.deadline, "PPP") : "No deadline"}
                        </TableCell>
                        <TableCell className={task.completed ? 'line-through text-muted-foreground' : ''}>
                         {/* Use formatTimestamp helper */}
                        {task.reminder ? formatTimestamp(task.reminder, "Pp") : (
                            <Popover>
                        ...
                        <Tooltip>
                            ...
                            <Button variant="ghost" size="icon" aria-label="Edit Task" disabled={task.completed || true /* Disable edit for now */}>
